...
const parseInput = (input: PuzzleInput): string[][] =>
...
const check =
  ...
  (
    { x, y }: Point,
    getNextPoint: (p: Point) => Point,
    [toMatch, ...rest]: string[],
  ) =>
    grid?.[y]?.[x] === toMatch &&
    (!rest.length || check(grid)(getNextPoint({ x, y }), getNextPoint, rest));
